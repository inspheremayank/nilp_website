...
import { VTRegisterSchema } from '@/schemas/formSchemas';
import InputField from '@/components/Form/InputField';
import SelectField from '@/components/Form/SelectField';
import Button from '@/components/Button';
import RadioField from '@/components/Form/RadioField';
import { AppContext } from '@/context/AppContext';
import { GET_DISTRICT, EXISTS_ENDPOINT } from '@/config/apiConfig';
...
const RegisterVtForm = (props: any) => {
  const { vtAgeOptions } = useContext(AppContext);
  ...
  const initialValues = {
    ...
    district: props.district != undefined ? props.district : '',
    education: '',
    vt_type: '',
    no_of_beneficiaries_to_be_taught: '',
    mark_as: CONSTANTS.MARK_AS_VT,
    school: props.school != undefined ? props.school : '',
    relation: '2',
  };
  ...
  const benerficiariesRef = useRef<any>('');

  const t = useTranslations();
  const router = useRouter();
  const { stateDropdown } = useContext(AppContext);
  const { setEncryptDataValue } = useContext(AppContext);
  ...
  const restrictDigitsFunc = (event: any) => {
    restrictDigits(event);
  };

  // limit length
  const limitLenthFunc = (event: any) => {
    const limit = 3;
    handleLimitLength(event, benerficiariesRef, limit);
  };
  ...
  const validate = (values: any): FormikErrors<any> => {
    ...
    if (!trimmedMotherName && !trimmedFatherName) {
      const errorMessage = "mother's name or father's/husband's name  required";
    ...
    if (values.no_of_beneficiaries_to_be_taught) {
      const number = parseInt(values.no_of_beneficiaries_to_be_taught, 10);
      if (number < 1) {
        errors.no_of_beneficiaries_to_be_taught =
    ...
    <>
      <div className="container">
        <Formik
          ...
          validationSchema={VTRegisterSchema}
          validate={validate}
            ...
            if (validateCaptcha(user_captcha)) {
              ...
              const response = await serverRequest(
                ...
                if (response.status == CONSTANTS.STATUS_SUCCESS) {
                  setIsLoader(false);
                  setEncryptDataValue(values);
                  router.push('/vt-tagging-learners');
                  // const vtFormValue = {
                  //   mobile: encryptedData.mobile,
                  //   state: encryptedData.state,
                  //   district: encryptedData.district,
                  //   isSearch: false,
                  //   mediumOfInstructions:
                  //     encryptedData.course_taught_in_language,
                  // };
            ...
            return (
              <Form onSubmit={handleSubmit}>
                <div className="row form_grider d1">
                  ...
                  <div className="col-12 col-xl-3 col-lg-4">
                    <SelectField
                      label={`${t('global.education')}*`}
                      placeholder={`${t('global.educationplaceholder')}`}
                      ...
                      options={education}
                      reference={selectEducationRef}
                      onChange={(selectEducation: any) => {
                        setFieldValue(
                          'education',
                          selectEducation ? selectEducation.value : ''
                        );
                      }}
                      onBlur={handleBlur}
                    />
                  ...
                  <div className="col-12 col-xl-3 col-lg-4">
                    <SelectField
                      label={`${t('global.VTtype')}*`}
                      placeholder={`${t('global.VTtypeplaceholder')}`}
                      errors={errors.vt_type}
                      touched={touched.vt_type}
                      onKeyDown={restrictDropdown}
                      name="vt_type"
                      value={values.vt_type}
                      options={teacher_type}
                      reference={selectVtTypeRef}
                      onChange={(selectVtType: any) => {
                        setFieldValue(
                          'vt_type',
                          selectVtType ? selectVtType.value : ''
                        );
                      }}
                      onBlur={handleBlur}
                    />
                  ...
                  <div className="col-12 col-xl-3 col-lg-3">
                    <InputField
                      label={`${t('global.beneficiaries')}*`}
                      type="text"
                      placeholder={`${t('global.beneficiariesplaceholder')}`}
                      name="no_of_beneficiaries_to_be_taught"
                      value={values.no_of_beneficiaries_to_be_taught}
                      onChange={handleChange}
                      onBlur={handleBlur}
                      errors={errors.no_of_beneficiaries_to_be_taught}
                      touched={touched.no_of_beneficiaries_to_be_taught}
                      onKeyDown={limitLenthFunc}
                      reference={benerficiariesRef}
                    />
                  </div>
